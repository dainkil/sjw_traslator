...
CONDITIONS = {
    ...
    'hybrid':        'results300_hybrid.jsonl',
}
...
for cond, ld in loaded.items():
    ...
    results[cond] = (bc, cc, bm, cm, ner)

b0 = results['baseline']
lines = [
    '=' * W,
    f'baseline / few-shot / few+nerfix / hybrid | n={len(common)} (300개 완전 비교)',
    '=' * W,
    f"{'':18s}  {'BLEU(c)':>9}  {'chrF(c)':>9}  {'BLEU(m)':>9}  {'chrF(m)':>9}  {'NER':>7}",
    '-' * W,
]
for cond, (bc, cc, bm, cm, ner) in results.items():
    lines.append(f'  {cond:16s}  {bc:>9.2f}  {cc:>9.2f}  {bm:>9.2f}  {cm:>9.2f}  {ner:>7.3f}')
lines.append('-' * W)
for cond in ['few-shot', 'few+nerfix', 'hybrid']:
    if cond in results:
        bc, cc, bm, cm, ner = results[cond]
        lines.append(f'  Δ {cond:14s}  {bc-b0[0]:>+9.2f}  {cc-b0[1]:>+9.2f}  {bm-b0[2]:>+9.2f}  {cm-b0[3]:>+9.2f}  {ner-b0[4]:>+7.3f}')
lines.append('=' * W)
print('\n' + '\n'.join(lines))
