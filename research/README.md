# 지식 주입형 언어 모델 기반 승정원일기 번역 모델 연구 (선행 연구)

> 이 문서는 선행 연구의 기록이다. 저장소는 현재 이 연구를 운영 가능한 번역 서빙 시스템으로 전환하는 프로젝트로 재편되었다 — 루트 [README](../README.md)와 [PROJECT_PLAN.md](../PROJECT_PLAN.md) 참조.
...
**KB 엔티티 링킹 — 3단계 구조** (구현 코드 `kb/data_ContextInjection.py` 기준)
```
1단계: 표층형으로 역색인 조회 → 후보군 생성 (단일 후보면 즉시 확정)
↓
2단계: 활동 시기 필터 — 활동_시작 ≤ 문서 연도인 후보만 유지
↓
3단계: 관직명 교차 검증 — 후보의 관직_리스트가 문맥에 등장하면 확정
↓
모호 케이스: 상위 후보 3명 + 경고 플래그로 LLM에 전달
```
> 초기 README에 기술됐던 "글자 교집합/합집합" 캐스케이드는 구현되지 않은 초안이었다. 위가 실제 검증된 알고리즘이다 (경위: `docs/adr/015-entity-linking-canon.md`).
---
...
[지식 주입형 언어 모델 기반 《승정원일기》 번역 모델 연구.pdf](<./docs/지식 주입형 언어 모델 기반 《승정원일기》 번역 모델 연구.pdf>)
