"""
eval300 v4 (프롬프트 극치): eval300_1925.json 300개
output: ablation5way/results300_v4.jsonl
"""
...
MAX_CONCURRENT = 5
OUTPUT_FILE    = "ablation5way/results300_v4.jsonl"
...
async def main():
    ...
    print(f"전체: 300 | 완료: {len(done)} | 남은것: {len(remaining)}")
    ...
    start = time.time()
    await tqdm_asyncio.gather(*[process(e, i % len(clients)) for i, e in enumerate(remaining)], desc="v4 번역")
    out_f.close()
