...
| M3 템플릿 슬롯 캐싱 | 진행 중 (S1~S3 완료, 2026-09-11) | S1 슬롯 도메인 로직 + ADR-009 / S2 L1 캐시 라이브 / S3 L2 캐시 라이브 — 인물만 다른 문장이 LLM 0회, fallback 실증 |
| M4 NER 비용 라우팅 | 예정 | |
...
- **다음: S4 — 인조 1년치(`A01-*`) 히트율 시뮬레이션 + 캐시 on/off 비용 비교표.**
  **골든셋 300문장에는 정확히 같은 틀의 문장 쌍이 0건**이다(층화 표본이므로 당연) — 실제 히트율은
  1년치 전수로만 나온다. `template_hash`가 L2 off에서도 쌓이므로 **기능을 켜지 않고** DB 집계로
  시뮬레이션할 수 있다: `GROUP BY template_hash HAVING count(*) > 1`.
  단, 1년치 전수는 `malmoi/` 원천 데이터가 있는 컴퓨터에서만 가능하다(§4 참조).
- 이후: **S5** L2 비열등 판정(chrF −2.0, 확정 인명 재현율 하락 0) 후 활성화 결정.
