...
| M3 템플릿 슬롯 캐싱 | 진행 중 (S1~S4 완료, 2026-09-11) | S1 슬롯 로직 + ADR-009 / S2 L1 라이브 / S3 L2 라이브 / S4 히트율 실측 — **인조 전수 L1+L2 17.36%**, 비용 비교표 |
| M4 NER 비용 라우팅 | 예정 | |
...
- **다음: S5 — L2 비열등 판정을 정식 수행하고 수치를 기록한다** (활성화 결정은 위에서 이미 내렸다).
  목적이 "켜도 되는지 판단"에서 **"끈 결정의 근거를 수치로 남기는 것"** 으로 바뀌었다.
  판정기는 `eval/score_db.py`가 종료 코드 계약을 갖고 있다. L2 히트 결과의 chrF를 재려면 **동일
  문장쌍을 L2 on/off로 각각 번역해 대조**해야 한다(층화 n=60 × 3회 = LLM 호출 약 180회 —
  flash-lite로 감당 가능). 표본 풀은 S4에서 L2 히트가 난 1,392쌍이다.
  결과가 "비열등 통과"로 나와도 위 결정은 유지한다 — 판정은 품질 축이고, 결정은 수혜 크기 축이다.
